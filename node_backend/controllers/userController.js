...
exports.setNewUserImage = catchAsyncError(async (req, res, next) => {
    const { new_user_image, email } = req.body;
    const user = await User.findOne({ email: email });
    const newResponse = await axios.post("http://127.0.0.1:5000/capture", { new_user_image })
    user.images = newResponse.data;
    await user.save();
    res.status(200).json({
        success: true,
        message: "new image added successful"
    });
})
